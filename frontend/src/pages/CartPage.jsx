...
const CartPage = () => {
  ...
  // Aktualizácia celkovej ceny pri zmene položiek v košíku
  useEffect(() => {
  ...
  const handleRemove = async (itemId) => {
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/cart/${itemId}`, {
        method: 'DELETE',
        headers,
      });
      refreshCart(); // automaticky načíta nový košík
    } catch (err) {
  ...
  const handleQuantityChange = async (itemId, newQuantity) => {
    ...
    try {
      ...
      refreshCart(); // aktualizuje aj celkový stav
    } catch (err) {
  ...
  if (cartItems.length === 0)
    return <p className="p-4 pt-8 text-red-500 text-center text-xl">Your cart is empty.</p>;
