          ...
          <div className="space-y-7 md:px-10">
            {filteredOrders.map((order) => {
              ...
              const subtotal = itemsArray.reduce(
                (sum, item) => sum + item.quantity * parseFloat(item.item_price || 0),
                0
              );
              const delivery = parseFloat(order.delivery_price || 0);
              const pointsValue = parseFloat(order.used_points || 0) * 0.1;
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-2">
                    <div className="text-sm text-center text-blue-100 mt-2">
                      {order.created_at
                        ? `${new Date(order.created_at).toLocaleDateString('sk-SK')} ${new Date(order.created_at).toLocaleTimeString('sk-SK', { hour: '2-digit', minute: '2-digit' })}`
                        : 'N/A'}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row md:justify-between md:gap-10">
                    <div className="flex-1 space-y-2 mb-6 md:mb-0">
                      <p><strong className="text-blue-200">User email:</strong> {order.user_email || 'N/A'}</p>
                      <p><strong className="text-blue-200">Full name:</strong> {order.full_name || 'N/A'}</p>
                      <p><strong className="text-blue-200">Profile email:</strong> {order.profile_email || 'N/A'}</p>
                      <p><strong className="text-blue-200">Address:</strong> {order.address || 'N/A'}, {order.city || 'N/A'} {order.postal_code || ''}</p>
                      <div className="mt-4 space-y-1">
                        <p><strong className="text-blue-200">Payment method:</strong> {order.payment_method || 'N/A'}</p>
                        <p><strong className="text-blue-200">Used points:</strong> {order.used_points || 0}</p>
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          <ul className="list-none list-inside space-y-1 text-sm md:text-[14px]">
                            {itemsArray.map((item, index) => {
                              ...
                              return (
                                <li key={index}>
                                  {item.product_name || 'Unknown'} – {item.quantity}×{price.toFixed(2)} € = {total.toFixed(2)} €
                                </li>
                              );
                            })}
                          </ul>
                          ...
                          <div className="mt-4 space-y-1 text-right">
                            ...
                            <p>
                              ...
                              {order.delivery_method || 'N/A'} ({delivery.toFixed(2)} €)
                            </p>
                            <p>
                              <span className="text-blue-200">- Used points: </span>
                              -{order.used_points || 0} ({pointsValue.toFixed(2)} €)
                            </p>
