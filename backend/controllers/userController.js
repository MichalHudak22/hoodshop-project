...
const verifyEmail = async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT * FROM email_verification_tokens WHERE token = ?', [
      token,
    ]);
    if (rows.length === 0) return res.status(400).json({ error: 'Token neplatný alebo už použitý' });

    const tokenData = rows[0];
    const now = new Date();
    const expiresAt = new Date(tokenData.expires_at);
    if (now > expiresAt) return res.status(400).json({ error: 'Token expiroval' });
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (user.is_verified === 0) {
      registerFailedAttempt(email);
      return res.status(403).json({ error: 'Email is not verified. Please check your email.' });
    }
